...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=DM+Sans:ital,wght@0,300;0,400;0,500;0,600;1,400&family=Cormorant+Garamond:wght@400;500;600;700&display=swap" rel="stylesheet">
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .sidebar-divider { border-color: rgba(255,255,255,0.06); }
        ...
        .stat-tag-teal   { background: #f0fdfa; color: #0f766e; }
        .stat-tag-amber  { background: #fef3c7; color: #b45309; }
        .stat-tag-rose   { background: #ffe4e6; color: #be123c; }
        ...
        .alert { animation: slideIn 0.22s ease; }
        ...
        @media (max-width: 1023px) {
            .sidebar { transform: translateX(-100%); transition: transform 0.24s cubic-bezier(.4,0,.2,1); }
            .sidebar.open { transform: translateX(0); }
            main.main-content { margin-left: 0 !important; padding-top: 5rem; }
        }
        ...
        ::-webkit-scrollbar-thumb { background: rgba(22,33,62,0.15); border-radius: 2px; }
...
<?php if (isLoggedIn()): ?>
    ...
    <div id="topbar" style="display:none" class="fixed top-0 left-0 right-0 z-40 px-4 py-3 flex items-center gap-3">
        <button id="hamburger" class="text-white/60 hover:text-white w-8 h-8 flex items-center justify-center transition-colors">
        ...
        <aside class="sidebar w-[240px] min-h-screen fixed left-0 top-0 z-30 flex flex-col">
            ...
            <div class="px-5 py-5 sidebar-divider border-b">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <p class="text-white font-semibold text-sm leading-tight tracking-tight truncate"><?= APP_NAME ?></p>
                        <p class="text-white/28 text-[11px] mt-0.5 truncate" style="color:rgba(255,255,255,0.28)"><?= APP_SCHOOL ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="px-5 py-4 sidebar-divider border-b">
                <div class="flex items-center gap-3">
                    ...
                    <div class="overflow-hidden">
                        <p class="text-white/85 text-[13px] font-medium truncate"><?= htmlspecialchars($user['nama']) ?></p>
                        <p class="text-white/30 text-[11px] capitalize"><?= $user['role'] ?><?= $user['no_anggota'] ? ' · ' . $user['no_anggota'] : '' ?></p>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 px-3 py-4 overflow-y-auto space-y-0">
                <?php if (isAdmin()): ?>
                    ...
                    <a href="/perpustakaan/pages/admin/dashboard.php"
                ...
                <?php else: ?>
                    ...
                    <a href="/perpustakaan/pages/siswa/denda.php"
                        ...
                        if ($cntDendaSiswa > 0): ?>
                            <span class="ml-auto text-[10px] bg-rose-500 text-white px-1.5 py-0.5 rounded-full font-bold min-w-[18px] text-center"><?= $cntDendaSiswa ?></span>
                        <?php endif; ?>
                    </a>
                    <a href="/perpustakaan/pages/siswa/profil.php"
            ...
            <div class="px-3 py-4 border-t sidebar-divider">
                <a href="/perpustakaan/logout.php"
        ...
        <main class="main-content ml-[240px] flex-1 min-w-0 p-7">
            <?php if ($flash): ?>
                <div class="alert mb-5 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2
                    ...
                    <i class="fas <?= $flash['type'] === 'success' ? 'fa-circle-check text-emerald-500' : 'fa-circle-exclamation text-red-500' ?>"></i>
